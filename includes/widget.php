...
add_action( 'widgets_init', 'gfv_load_widgets' );
/**
 * Register widgets for use in the Genesis theme.
 *
 * @since 1.7.0
 */
function gfv_load_widgets() {

	register_widget( 'SD_Genesis_Featured_Video' );

}

class SD_Genesis_Featured_Video extends WP_Widget {
	/**
	...
	public function __construct() {
		...
		$widget_ops = array(
			'classname'   => 'featured-content featuredpost featured-videos-for-genesis',
			'description' => __( 'Displays featured posts with videos', 'genesis' ),
		);

		$control_ops = array(
			'id_base' => 'featured-video',
			'width'   => 505,
			'height'  => 350,
		);

		parent::__construct( 'featured-video', __( 'Featured Posts with Videos', 'genesis' ), $widget_ops, $control_ops );
	...
	public function widget( $args, $instance ) {
		...
		if ( have_posts() ) : while ( have_posts() ) : the_post();
			 ...
			 * The featured video should take precedence, and fall back to the featured image.
			 * We also want something to show if there is no featured video, but there is a featured image.
			 */
			if ( $instance['show_image'] && $instance['show_video'] ){
				$toggle_vid_img = 'img';
				if ( $video ){
					$toggle_vid_img = 'vid';
				}
			...
			if ( $toggle_vid_img ){
				switch ( $toggle_vid_img ) {
					case 'vid':
						if ( $video && $instance['show_video'] ){
							$align = in_array( $instance['video_alignment'], array( 'none', 'left', 'right' ) ) ? $instance['video_alignment'] : 'none';
							$dimensions = array(
								'width' 	=> absint( $instance['video_width'] ),
								'height'	=> absint( $instance['video_height'] ),
								'align'		=> $align
							);
							echo gfv_video_image( $dimensions );
						}
						break;
	...
	public function form( $instance ) {
		...
		<div class="genesis-widget-column genesis-widget-column-right">
			...
			<div class="genesis-widget-column-box">
				...
				<p>
					<label for="<?php echo esc_attr( $this->get_field_id( 'video_width' ) ); ?>"><?php _e( 'Video Width', 'genesis' ); ?>:</label>
					<input type="text" id="<?php echo esc_attr( $this->get_field_id( 'video_width' ) ); ?>" name="<?php echo esc_attr( $this->get_field_name( 'video_width' ) ); ?>" value="<?php echo esc_attr( $instance['video_width'] ); ?>" size="2" />
				</p>
				<p>
					<label for="<?php echo esc_attr( $this->get_field_id( 'video_height' ) ); ?>"><?php _e( 'Video Height', 'genesis' ); ?>:</label>
					<input type="text" id="<?php echo esc_attr( $this->get_field_id( 'video_height' ) ); ?>" name="<?php echo esc_attr( $this->get_field_name( 'video_height' ) ); ?>" value="<?php echo esc_attr( $instance['video_height'] ); ?>" size="2" />
				</p>
				<p>
					<label for="<?php echo esc_attr( $this->get_field_id( 'video_alignment' ) ); ?>"><?php _e( 'Video Alignment', 'genesis' ); ?>:</label>
					<select id="<?php echo esc_attr( $this->get_field_id( 'video_alignment' ) ); ?>" name="<?php echo esc_attr( $this->get_field_name( 'video_alignment' ) ); ?>">
						<option value="none" <?php selected( 'none', $instance['video_alignment'] ); ?>><?php _e( 'None', 'genesis' ); ?></option>
						<option value="left" <?php selected( 'left', $instance['video_alignment'] ); ?>><?php _e( 'Left', 'genesis' ); ?></option>
						<option value="right" <?php selected( 'right', $instance['video_alignment'] ); ?>><?php _e( 'RIght', 'genesis' ); ?></option>
					</select>
				</p>
			</div>
